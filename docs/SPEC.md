...
* **Entity Resolution:**
  ...
  * ベクトルベース解決（埋め込み類似度による重複検出）とLLMベース解決（曖昧なケースの検証）を併用。
* **Community Detection (コミュニティ検出):**
  * **Leidenアルゴリズム**を採用（Louvainより高速で連結性を保証）。
  * 階層的なコミュニティ構造を維持し、各レベルで自然言語要約を生成。
  * グローバル検索（データセット全体に関する質問）に対応。
* **Hyper-Index:**
...
* **Auto-Graph Construction (自動グラフ構築):** 【最大の差別化機能】
  ...
  * **Lazy Graph Construction:** 取り込み時はベクトル化を優先し、詳細なグラフ構築はバックグラウンドまたは初回アクセス時に遅延実行する。
* **Idempotency & Dedup:**
...
* **ローカル検索 (Local Search):**
  * 特定エンティティ中心の探索（1-2 hop）に最適化。
* **DRIFT検索 (Dynamic Reasoning via Iterative Feedback and Traversals):**
  * 初期検索で情報が不足している場合、部分的な証拠が示す方向にグラフ探索を動的に拡張。
  * 人間の調査的な読書プロセスを模倣したフィードバックループ。
* **グローバル検索 (Global Search):**
  * データセット全体に関する質問（例：「主要なテーマは何か？」）に対応。
  * 事前計算されたコミュニティ要約を活用したMap-Reduceスタイルの回答生成。
* **Explain Plan:**
...
* **search_mode** (string, optional): `local` | `global` | `drift` | `auto` (default)
* **model_id** (string, optional)
* **snapshot_id** (string, optional)

**search_mode = auto の選択基準 (暫定):**
* 「全体/主要テーマ/総括」などの意図が強い場合 → `global`
* 具体的エンティティ中心の質問 → `local`
* 初期検索で十分な証拠が得られない場合 → `drift` にフォールバック
...
* **増分インデックス (Incremental Indexing):**
  * 新規ドキュメント追加時、インデックス全体を再構築せずローカルでグラフを更新。
  * コミュニティ要約も増分的に更新（リアルタイムGraphRAG）。

### 3.5. エージェンティックワークフロー対応

* **セッショングラフ:**
  * ユーザーの会話やエージェントのタスク期間中のみ存在する一時的なサブグラフ。
  * AIが会話の構造を明示的に「記憶」することを可能にする。
* **ワーキングメモリ:**
  * エージェントが中間的な思考やタスク状態をグラフに書き込み可能。
  * 低レイテンシ（ミリ秒単位）の読み書きを提供。

### 3.6. フレームワーク統合

* **LlamaIndex / LangChain 統合:**
  * `GraphStore` および `VectorStore` インターフェースのネイティブ実装を提供。
  * `PropertyGraphIndex` パターンに準拠。
* **SDK:**
  * Python, TypeScript, Rustクライアントを提供。

### 3.7. 可視化 (Visualization)

* **グラフエクスプローラー:**
  * フォースダイレクテッドレイアウト（力学モデル）による2D/3Dグラフ表示。
  * コミュニティのクラスタリング表示とズーム機能。
  * AIチャットとのインタラクティブ連携（クリックでノード詳細、エッジ追跡）。
...
response = db.query(
    ...
    search_mode="local",  # local | global | drift | auto
    depth=2,
...
* **自動グラフ構築の精度とコスト** (GraphRAGはインデックス作成がベクトルRAGの50-100倍高価)
* **GPUメモリ逼迫時の性能劣化**
* **モデルドリフトによる関係抽出精度の低下**
* **クエリの説明可能性不足による運用リスク**
* **レイテンシ** (GraphRAGは5-10秒かかる可能性、インスタント検索向きではない)

### 6.2. 解決策と検証方針

* **遅延グラフ構築（Lazy Graph Construction / LazyGraphRAG）**
  * データ投入時はまずベクトル化だけ行い（高速）、バックグラウンドプロセスで、またはそのデータが初めてアクセスされた時に、詳細なグラフ関係性を抽出・構築する仕様とする。
* **FastGraphRAG アプローチ**
  * 全コミュニティを要約する代わりに、PageRankで「最も重要な上位10%のノード」を特定し、それらのみを要約。
* **コスト削減のためのローカルSLM活用**
  * Triplex (Phi-3ベース、3.8Bパラメータ) や GLM-4-Flash のような特化型小規模モデルを使用。
  * 汎用LLM (GPT-4) 比で抽出コストを最大98%削減可能。
* **キャッシュ/退避ポリシー**
  * VRAM常駐データの優先度と退避ルールを明文化。
* **増分インデックス**
  * リアルタイム更新をサポートし、バッチ再構築を回避。
* **評価指標の設定**
