...
- GPU-First / GPUDirect は高難度のため、**CPU経路を先に実装**し、GPU-First Persistence / Zero-Copy Access は将来の性能拡張として段階導入する（Issue #51）。PR-17.3 で CPU ベースの ANN/検索経路は成立したが、`docs/SPEC.md` が掲げる GPU 常駐・GPUDirect 前提のストレージ階層は未着手のため、現時点では必須要件から切り離して別トラックで扱う。
...
* Depends on: PR-13, PR-14, PR-14.8, PR-17.1, PR-17.2, PR-17.3, PR-17.4, PR-17.5
...
* Depends on: PR-06, PR-14.8

- [ ] `MockEntityExtractor` を本番デフォルトから外し、テスト専用実装へ隔離
- [ ] ルールベース抽出 + 軽量SLM抽出（Triplex/GLM-4-Flash等）のハイブリッド抽出器を `ModelRegistry` で切替可能にする (Issue #48)
- [ ] ローカルで動作する実 SLM（Triplex / GLM-4-Flash 等）への置換を完了し、外部 API 依存なしで抽出が実行できることを確認
...
- [ ] 実世界の複雑なデータセット（複数エンティティ・関係を含む文書）を用いた GraphRAG 精度検証が通過する (Issue #52)
  - **進捗メモ (Issue #52, 部分達成):** マルチホップ GraphRAG 推論と DRIFT 探索を検証する包括的統合テスト群を `query/tests/multihop_graphrag_test.rs` に追加（実世界風サプライチェーン KG フィクスチャ、深さ 1〜4 の到達性・BFS 経路復元・サイクル終端・切断成分の分離・関係フィルタ・top_k プルーニング・DRIFT モード選択とカバレッジ・Auto→DRIFT フォールバック・再現性、計13テスト）。上記 Done Criteria の「精度検証」自体は実抽出器（PR-17.4 / Issue #48）の完成後に闭合させる。

---

## PR-17.5: セキュリティと推論精度の高度化 (NEW)
...
- PR-14.8 → PR-17.4/15 (NEW: 環境整備とビルド安定化)
- PR-16 → 商用化フェーズのHA/冗長性
- PR-17 (Epic) → PR-17.1/17.2/17.3/17.4/17.5
- PR-17.1 → PR-16/17.2/15
- PR-17.2 → PR-15
- PR-17.3 → PR-15
- PR-17.4 → PR-17.5/15
- PR-17.5 → PR-15 (NEW: セキュリティと推論精度の高度化)
