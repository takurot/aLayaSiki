...
- GPU-First / GPUDirect は高難度のため、**CPU経路を先に実装**し、GPUはティアリングの拡張として段階導入する。CPU経路でのロジックはPR-17.3時点で完成しており、GPU対応は将来の性能拡張オプションとして位置づける（現時点での必須要件ではない）。
...
* Depends on: PR-13, PR-14, PR-17.1, PR-17.2, PR-17.3, PR-17.4, PR-17.5
